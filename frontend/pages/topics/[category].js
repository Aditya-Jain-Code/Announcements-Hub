...
export default function CategoryPage() {
    ...
    useEffect(() => {
        ...
        const fetchNewsData = async () => {
            try {
                const res = await axios.get(`/api/getnews?newscategory=${category}`);
                ...
                <div className="container">
                    <div className="category_title">
                        <div className="flex gap-1">
                            <h1>{loading ? <div>Loading...</div> : category}</h1>
                            <span>{loading ? <div>0</div> : publishedNews.filter(news => news.newscategory).length}</span>
                        </div>
